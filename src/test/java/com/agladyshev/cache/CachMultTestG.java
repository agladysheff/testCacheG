...
public class CachMultTestG {
    ...
    public void multPutGet() {
        Integer n = 20;
        Integer diff = 150;
        List<List<Integer>> listListResult = IntStream.range(0, n).mapToObj(x -> new ArrayList<Integer>())
                .collect(Collectors.toList());
        ExecutorService executorService = Executors.newFixedThreadPool(5);
        List<Integer> listExpect = IntStream.range(0, n * diff).boxed().collect(Collectors.toList());
        IntStream.range(0, n).mapToObj(x -> executorService.submit(() -> asyncPutGet(x, diff, listListResult.get(x))))
                .collect(Collectors.toList()).forEach(x -> {
            while (!x.isDone()) {
            }
        });
        List<Integer> listResult = listListResult.stream().flatMap(Collection::stream)
